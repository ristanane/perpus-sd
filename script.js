...
async function muatDataAwal() {
    try {
        ...
        // Panggil inisialisasi di sini agar data sudahhhhhh terisi
        inisialisasiSistem(); 
...
function inisialisasiSistem() {
    // 1. Setup Autocomplete untuk Siswa
    ...
    // 2. Setup Khusus Buku (Tanpa fungsi generik, dijamin responsif)
    if (inputBuku && bukuSuggestions) {
        function renderDropdownBuku() {
            ...
            if (filtered.length === 0) {
                ...
                // Jika tidak ada di master -> BUKU BARU
                if (pengarangField) {
            ...
            filtered.forEach(b => {
                ...
                div.innerHTML = `<strong>${b[1]}</strong> <small style="color:var(--text-muted);">(ID: ${b[0]} - Pengarang: ${b[2]})</small>`;
                
                // Gunakan click langsung dengan pengaman
                div.addEventListener('mousedown', function(e) {
                    e.preventDefault();
                    
                    // Isi data ke form
                    inputBuku.value = b[1];             // Judul Buku
                    if (idBukuField) idBukuField.value = b[0];       // ID Buku
                    if (pengarangField) {
        ...
        inputBuku.addEventListener('input', renderDropdownBuku);
        inputBuku.addEventListener('focus', renderDropdownBuku);
        inputBuku.addEventListener('blur', () => setTimeout(() => { bukuSuggestions.style.display = 'none'; }, 250));
    }    
    // 4. Setup Autocomplete untuk Tamu
